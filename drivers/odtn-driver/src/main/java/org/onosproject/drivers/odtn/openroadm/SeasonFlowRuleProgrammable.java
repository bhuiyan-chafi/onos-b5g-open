 ...
 */
public class SeasonFlowRuleProgrammable
    ...
    public Collection<FlowRule> applyFlowRules(Collection<FlowRule> rules) {
        ...
        FlowRule rule = rules.iterator().next();
        if (rule.selector().criteria().size() == 1) {
            log.info("[SEASON] a rule has been received with only ports...");
            log.info("[SEASON] dummyOchSignal will be used...");
        }
